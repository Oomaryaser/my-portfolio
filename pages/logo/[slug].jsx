...
export default function LogoGallery() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setImages([]); setLoaded([]); setCat(null);
      } finally {
        setLoading(false);
      }
    })();
  ...
  const low  = src => src.startsWith('data:') ? src : `${src}${src.includes('?') ? '&' : '?'}w=480&q=40`;
    ...
    <div dir="rtl" className="font-[Beiruti] flex flex-col min-h-screen bg-white text-gray-900">
      ...
      <main className="flex-1 w-full max-w-none p-4 pt-24">
        ...
        {!loading && images.length > 0 && (
          <motion.section variants={container} initial="hidden" animate="visible" className="columns-1 sm:columns-2 md:columns-3 lg:columns-4 gap-4 space-y-4">
            {images.map((img, i) => (
              <motion.div key={img.id} variants={item} className="break-inside-avoid rounded-xl overflow-hidden relative group">
